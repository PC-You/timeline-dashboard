...
export function importState(payload) {
    ...
    // Restore column config (v0.5.0+). If absent in older exports, ingest() will
    // auto-detect defaults from data; explicit overrides take precedence over them.
    if (payload.columnConfig) {
        state.columnConfig = payload.columnConfig;
    }
    ...
    if (payload.settings) {
        ...
        if (payload.settings.developerMode != null) {
            settings.developerMode = payload.settings.developerMode;
        ...
        if (devCb) devCb.checked = settings.developerMode;
    }
